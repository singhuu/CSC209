...
int main(int argc, char **argv)
{
    ...
    // Your code goes here
    char *temp_greet = argv[1];
    // First we can combine both of them into a single array [concat]
    char *arr = malloc(sizeof(char) * (strlen(argv[1]) + strlen(argv[2]) + 1 + 1));
    strcpy(arr, temp_greet);
    strcat(arr, " ");
    strcat(arr, name);
    // Then we will truncate based on the 20 character limit
    strncpy(greeting, arr, 20);
    // Ensure that it has a null terminator
    greeting[19] = '\0';
    printf("%s\n", greeting);
    return 0;
}
